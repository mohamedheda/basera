        ...
        <x-cards.page-card>
            <x-slot name="header">
                <div class="card-title">
                    @lang('dashboard.Create') @lang('dashboard.Subscription Package')
                </div>
            </x-slot>
            <x-form.form-component :route="route('subscription-packages.store')" method="POST">
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="duration_type" id="duration_type" class="form-select" required>
                            ...
                            <option value="monthly">@lang('dashboard.Monthly')</option>
                            <option value="semi_annual">@lang('dashboard.Semi Annual')</option>
                            <option value="annual">@lang('dashboard.Annual')</option>
                        </select>
                        @error('duration_type')
                    ...
                    <div class="col-md-3">
                        <x-input.input-field name="currency" label="{{ __('dashboard.Currency') }}"
                            placeholder="{{ __('dashboard.Enter currency') }}" required="true" value="ر.س" />
                    </div>

                    <div class="col-md-12">
                        <label for="description" class="form-label">@lang('dashboard.Description')</label>
                        <textarea name="description" id="description" class="form-control" rows="3"
                            placeholder="{{ __('dashboard.Enter package description') }}"></textarea>
                        @error('description')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
